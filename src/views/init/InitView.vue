<template>
    ...
    <i class="fas fa-plus mr-1"></i> Agregar Censo
              ...
              <InitModalView v-if="showModal" :show-modal="showModal" :form-data="formData" @close-modal="closeModal"></InitModalView>
            </div>
  ...
  export default defineComponent({
    ...
    const route = router.currentRoute;
      ...
      const columns: ConfigColumns[] = [
        ...
        { data: "numero_integrantes", title: "N° Total de integrantes" },
    ...
    render: (data: string, type: string, row: any) => {
  ...
  },
        {
          ...
          render: (data: string, type: string, row: any) => {
            ...
            <router-link :to="'/home' + row.id">
              ...
              <button class="btn btn-warning" data-action="editar" data-id="${row.censo_id}">
      ...
      const fetchContacts = async () => {
        try {
          const response = await axios.get(
            `${import.meta.env.VITE_API_URL}/censo/list-custom`,
            headers
          );
      ...
      try {
        ...
        const nuevoCenso = response.data.data; // Datos del nuevo censo, incluido el UUID generado
        console.log('Censo creado:', nuevoCenso);

        // Redirigir a la página de detalle del censo recién creado
        router.push(`/ficha/${nuevoCenso.uuid}`); // Suponiendo que 'uuid' es el nombre del campo en el objeto del censo
      } catch (error) {
